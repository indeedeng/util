...
/**
 * A {@link CompressorStream} which works with 'block-based' based compression algorithms, as
 * opposed to 'stream-based' compression algorithms.
 *
 * <p>It should be noted that this wrapper does not guarantee that blocks will be sized for the
 * compressor. If the {@link Compressor} requires buffering to effect meaningful compression, it is
 * responsible for it.
 */
public class BlockCompressorStream extends CompressorStream {
     ...
     * @param compressionOverhead maximum 'overhead' of the compression algorithm with given
     *     bufferSize
     */
    public BlockCompressorStream(
            OutputStream out, Compressor compressor, int bufferSize, int compressionOverhead) {
        super(out, compressor, bufferSize);
        MAX_INPUT_SIZE = bufferSize - compressionOverhead;
    }

    /**
     * Create a {@link BlockCompressorStream} with given output-stream and compressor. Use default
     * of 512 as bufferSize and compressionOverhead of (1% of bufferSize + 12 bytes) = 18 bytes
     * (zlib algorithm).
     *
     * @param out stream
     * @param compressor compressor to be used
    ...
    /**
     * Write the data provided to the compression codec, compressing no more than the buffer size
     * less the compression overhead as specified during construction for each block.
     *
     * <p>Each block contains the uncompressed length for the block, followed by one or more
     * length-prefixed blocks of compressed data.
     */
