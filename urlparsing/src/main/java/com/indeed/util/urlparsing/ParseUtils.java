...
public final class ParseUtils {
    /**
     * Parses out an int value from the provided string, equivalent to
     * Integer.parseInt(s.substring(start, end)), but has significantly less overhead, no object
     * creation and later garbage collection required
     */
    ...
    /**
     * Parses out a long value from the provided string, equivalent to
     * Long.parseLong(s.substring(start, end)), but has significantly less overhead, no object
     * creation and later garbage collection required
     *
     * @throws NumberFormatException if it encounters any character that is not [-0-9].
     */
    ...
    /**
     * Parses out a float value from the provided string, more specialized than
     * Float.parseFloat(s.substring(start, end)) but has significantly less overhead, no object
     * creation and later garbage collection required Does not support parsing the strings "NaN",
     * "Infinity","-Infinity" and HexFloatingPointLiterals
     *
            ...
            if (c == 'N' || c == 'I') {
                ...
                while (i < end && j < charsToMatch.length) {
                    if (s.charAt(i) == charsToMatch[j]) {
                        ...
                        throw new NumberFormatException(
                                "Unsupported format, unable to parse input string ");
                    }
                }
                ...
                if (i == end
                        && j
                                == charsToMatch
                                        .length) { // this means we matched either NaN or Infinity
                    if (isNanStart) {
                        return Float.NaN;
                    } else if (sign == 1) {
                    ...
                    if (i + 11 < end
                        ...
                        if (val2 != INVALID_HEX
                            ...
                            if (codePoint < Character.MIN_SUPPLEMENTARY_CODE_POINT) {
                                ...
                                final char highChar =
                                        (char) ((offset >>> 10) + Character.MIN_HIGH_SURROGATE);
                                final char lowChar =
                                        (char) ((offset & 0x3ff) + Character.MIN_LOW_SURROGATE);
                                if (!inplace) {
